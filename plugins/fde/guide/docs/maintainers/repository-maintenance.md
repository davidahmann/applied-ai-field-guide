...
| `plugins/fde/` | Local personal distribution of the complete skill pack and bounded STDIO workspace | Keep generated skill copies in exact sync except for deterministic packaged-link rewriting, tools local and typed, immutable revisions and event history contained, the mutable projection atomic, and model-context and authority limits explicit |
...
| FDE plugin or MCP tool | Canonical skill parity, manifest and MCP metadata, input schema, path and size limits, idempotency, immutable history, model-context disclosure, local install and disable path, negative tests, README, AGENTS, llms, package scripts, and release version |
...
7. When the FDE plugin changes, run its official plugin validator in a disposable Python environment, install it locally from the trusted release tree, start a fresh task, and verify STDIO initialization plus one read and one contained write without customer data.
