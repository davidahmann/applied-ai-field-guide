...
import { FdeError } from "./workspace.mjs";
...
const serverInfo = { name: "fde-local-copilot", version: manifest.version };
  ...
  switch (message.method) {
    case "initialize":
      result(message.id, {
        ...
        serverInfo,
        instructions: "Local-only FDE engagement workspace and Guide navigation. Tools do not grant authority, open external sources, or prove customer or production decisions.",
      });
      return;
    case "ping":
      result(message.id, {});
    ...
    case "tools/call": {
      ...
      try {
        ...
        const known = caught instanceof FdeError;
        const payload = {
          status: "error",
          error: { code: known ? caught.code : "INTERNAL_ERROR", message: known ? caught.message : "The local FDE tool failed safely.", details: known ? caught.details : {} },
        };
        if (!known) process.stderr.write(`[fde-mcp] ${caught?.stack || caught}\n`);
...
process.stdin.on("end", () => {
  ...
  queue.catch((caught) => process.stderr.write(`[fde-mcp] ${caught?.stack || caught}\n`));
});
