...
1. Read the [control catalog](../../guide/controls/control-catalog.json), [release gates](../../guide/operations/release-gates.md), and [production service readiness template](../../guide/templates/production-service-readiness.md). When model or agent behavior is selected, also read the [solution-release contract](../../guide/schemas/solution-release.schema.json).
2. Resolve guide artifacts through [catalog.json](../../guide/catalog.json). Resolve model/agent candidate artifacts through the solution-release manifest; for a deterministic, optimization, or classical-ML-only system, use its target-software release record and equivalent architecture, test, provenance, deployment, rollback, and operating evidence without placeholder agent artifacts.
...
4. If the release claims a solution artifact, resolve it through the [solution portfolio](../../guide/solutions/README.md) and read only the selected business-flow pattern and optional vertical profile. Verify customer-specific gaps and non-claims; a design accelerator is not release evidence.
