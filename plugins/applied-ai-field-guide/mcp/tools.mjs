...
function ensureArray(value, field, { max = 100 } = {}) {
  ...
  if (!Array.isArray(value) || value.length > max) throw new GuideError("INVALID_INPUT", `${field} must be an array with at most ${max} entries`);
  return value;
}
...
function requireKnownRefs(state, values, field) {
  ...
  if (missing.length > 0) throw new GuideError("UNKNOWN_REFERENCE", `${field} contains unknown references`, { missing });
  return [...new Set(refs)];
}
...
function requireSourceRefs(state, values, field) {
  ...
  if (invalid.length > 0) throw new GuideError("REFERENCE_TYPE_MISMATCH", `${field} accepts source references only`, { invalid });
  return [...new Set(refs)];
}
...
function requireArtifactRefs(state, values, field) {
  ...
  if (invalid.length > 0) throw new GuideError("REFERENCE_TYPE_MISMATCH", `${field} accepts artifact references only`, { invalid });
  return [...new Set(refs)];
}
...
function bindReference(state, reference) {
  ...
  throw new GuideError("UNKNOWN_REFERENCE", `reference ${reference} cannot be bound`);
}
...
function requireAcyclicArtifactDependencies(state, artifactId, dependencies) {
  ...
  const visit = (node, trail) => {
    if (active.has(node)) throw new GuideError("DEPENDENCY_CYCLE", "artifact dependencies contain a cycle", { cycle: [...trail, node] });
...
function assertGloballyAvailableId(state, identifier, expectedKind) {
  ...
  if (kinds.length > 0 && !(kinds.length === 1 && kinds[0] === expectedKind)) {
    throw new GuideError("IDENTIFIER_CONFLICT", `${identifier} is already used by ${kinds.join(", ")}`);
  }
}

function validateSchemaValue(schema, value, field) {
  if (schema.type === "object") {
    if (!value || typeof value !== "object" || Array.isArray(value)) throw new GuideError("INVALID_INPUT", `${field} must be an object`);
    const properties = schema.properties || {};
    const unknown = Object.keys(value).filter((key) => !(key in properties));
    if (schema.additionalProperties === false && unknown.length > 0) throw new GuideError("INVALID_INPUT", `${field} contains unknown fields`, { unknown });
    for (const required of schema.required || []) {
      if (!(required in value)) throw new GuideError("INVALID_INPUT", `${field}.${required} is required`);
    }
    for (const [key, item] of Object.entries(value)) validateSchemaValue(properties[key], item, `${field}.${key}`);
    return;
  }
  if (schema.type === "string") {
    if (typeof value !== "string") throw new GuideError("INVALID_INPUT", `${field} must be a string`);
    if (schema.minLength !== undefined && value.length < schema.minLength) throw new GuideError("INVALID_INPUT", `${field} is too short`);
    if (schema.maxLength !== undefined && value.length > schema.maxLength) throw new GuideError("INVALID_INPUT", `${field} is too long`);
  } else if (schema.type === "integer") {
    if (!Number.isInteger(value)) throw new GuideError("INVALID_INPUT", `${field} must be an integer`);
    if (schema.minimum !== undefined && value < schema.minimum) throw new GuideError("INVALID_INPUT", `${field} is below its minimum`);
    if (schema.maximum !== undefined && value > schema.maximum) throw new GuideError("INVALID_INPUT", `${field} is above its maximum`);
  } else if (schema.type === "boolean") {
    if (typeof value !== "boolean") throw new GuideError("INVALID_INPUT", `${field} must be a boolean`);
  } else if (schema.type === "array") {
    if (!Array.isArray(value)) throw new GuideError("INVALID_INPUT", `${field} must be an array`);
    if (schema.minItems !== undefined && value.length < schema.minItems) throw new GuideError("INVALID_INPUT", `${field} has too few entries`);
    if (schema.maxItems !== undefined && value.length > schema.maxItems) throw new GuideError("INVALID_INPUT", `${field} has too many entries`);
    value.forEach((entry, index) => validateSchemaValue(schema.items, entry, `${field}[${index}]`));
  }
  if (schema.enum && !schema.enum.includes(value)) throw new GuideError("INVALID_INPUT", `${field} is outside the supported vocabulary`);
}
...
function requireRfc3339Timestamp(value, field) {
  ...
  if (!match) throw new GuideError("INVALID_INPUT", `${field} must be an RFC 3339 timestamp with an explicit timezone`);
  ...
  if (month < 1 || month > 12 || day < 1 || day > daysInMonth || hour > 23 || minute > 59 || second > 59 || offsetHour > 23 || offsetMinute > 59) {
    throw new GuideError("INVALID_INPUT", `${field} contains an invalid calendar date or time`);
  }
  ...
  if (Date.parse(candidate) !== expected) throw new GuideError("INVALID_INPUT", `${field} contains an invalid RFC 3339 timestamp`);
...
function requireEngagementModelContextPolicy(config, state) {
  if (state.engagement.classification === "restricted") {
    throw new GuideError("RESTRICTED_CONTENT_DENIED", "restricted engagements are unsupported at this model-visible boundary");
  }
  if (state.engagement.classification === "confidential" && !config.allowConfidentialModelContext) {
    throw new GuideError("MODEL_CONTEXT_POLICY_DENIED", "confidential engagement use requires allow_confidential_model_context in the machine-local plugin configuration");
  }
}
...
function nextMove(state) {
  ...
  if (fieldBlocker?.disposition === "defer") return { stage: "deferred", skill: "$run-ai-engagement", decision_stream_id: fieldBlocker.decision_stream_id, move: `Hold the field move for ${fieldBlocker.scope} until its recorded review trigger or an explicit same-stream supersession of decision ${fieldBlocker.decision_id}.` };
  if (fieldBlocker?.disposition === "review_required") return { stage: "field-review", skill: "$run-ai-engagement", decision_stream_id: fieldBlocker.decision_stream_id, move: `Obtain the recorded field review for ${fieldBlocker.scope} before explicitly superseding decision ${fieldBlocker.decision_id}.` };
...
export const toolDefinitions = [
  {
    name: "guide_search",
    description: "Search the configured Applied AI Field Guide snapshot by query and optional stage. Read-only, engagement-independent, local filesystem only; results are bounded excerpts and never authority or customer evidence.",
  ...
  },
  {
    ...
    description: "List bounded metadata for local applied-AI engagement workspaces so a fresh task can resume deliberately. Returns no source excerpts or artifact content and ignores non-engagement entries.",
...
export async function callTool(name, rawInput = {}) {
  ...
  if (!definition) throw new GuideError("TOOL_NOT_FOUND", `unknown Guide tool ${name}`);
  ...
  switch (name) {
    ...
    default: throw new GuideError("TOOL_NOT_FOUND", `unknown Guide tool ${name}`);
  }
}
...
async function engagementList(config, input) {
  ...
  for (const entry of entries) {
    try {
      ...
      invalid.push({ engagement_id: entry.name, error: error instanceof GuideError ? error.code : "INVALID_LOCAL_STATE" });
    }
...
async function artifactReadRevision(config, input) {
  ...
  if (!artifact) throw new GuideError("ARTIFACT_NOT_FOUND", `artifact ${artifactId} revision was not found`);
  const loaded = await readWorkspaceFile(config, input.engagement_id, artifact.relative_path, config.maxWriteBytes + 1);
  if (digest(loaded.body) !== artifact.file_digest) throw new GuideError("ARTIFACT_INTEGRITY_FAILURE", "saved artifact file no longer matches its immutable receipt");
...
async function sourceRegister(config, input) {
  ...
  if (excerpt && classification === "restricted") throw new GuideError("RESTRICTED_CONTENT_DENIED", "restricted source excerpts cannot be stored through this model-visible tool");
  if (classification === "confidential" && !config.allowConfidentialModelContext) {
    throw new GuideError("MODEL_CONTEXT_POLICY_DENIED", "confidential source metadata or excerpts require allow_confidential_model_context in the machine-local plugin configuration");
  }
  return mutateEngagement(config, input.engagement_id, input.operation_id, input, "source_registered", (state) => {
    ...
    if (classificationRank.get(classification) > classificationRank.get(state.engagement.classification)) {
      throw new GuideError("CLASSIFICATION_BOUNDARY_VIOLATION", `source classification ${classification} exceeds the ${state.engagement.classification} engagement boundary`);
    }
    assertGloballyAvailableId(state, sourceId, "source");
    if (state.sources.some(({ source_id: id }) => id === sourceId)) throw new GuideError("SOURCE_CONFLICT", `source ${sourceId} already exists`);
    const authorityStatus = requireString(input.authority_status, "authority_status", { max: 50 });
    if (!sourceAuthorityStatuses.has(authorityStatus)) throw new GuideError("INVALID_INPUT", "authority_status is outside the supported vocabulary");
    const source = {
      source_id: sourceId,
...
async function artifactSaveRevision(config, input) {
  ...
  if (!formats.has(format)) throw new GuideError("INVALID_INPUT", "format is not supported");
  const status = requireString(input.status, "status", { max: 30 });
  if (!artifactStatuses.has(status)) throw new GuideError("INVALID_INPUT", "artifact status is not supported");
  const content = requireString(input.content, "content", { max: config.maxWriteBytes });
  if (format === "json") {
    try { JSON.parse(content); } catch (error) { throw new GuideError("INVALID_JSON", `artifact content is not valid JSON: ${error.message}`); }
  }
  ...
  try {
    return await mutateEngagement(config, input.engagement_id, input.operation_id, input, "artifact_revision_saved", async (state, paths) => {
      ...
      if (priorRevisions.some((artifact) => artifact.artifact_type !== artifactType || artifact.format !== format)) {
        throw new GuideError("ARTIFACT_IDENTITY_CONFLICT", `artifact ${artifactId} cannot change artifact_type or format across revisions`);
      }
...
async function verifyValidatorRuntime(config, canonicalType) {
  ...
  for (const relative of files) {
    ...
    if (packagedDigest !== runtimeDigest) throw new GuideError("VALIDATOR_RUNTIME_DRIFT", `${relative} differs from the installed Applied AI Field Guide snapshot`);
...
async function decisionRecord(config, input) {
  ...
  if (!decisionKinds.has(kind)) throw new GuideError("INVALID_INPUT", "decision_kind is not supported");
  const disposition = requireString(input.disposition, "disposition", { max: 100 });
  if (!allowedDispositions.has(disposition)) throw new GuideError("INVALID_INPUT", "disposition is outside the bounded local vocabulary");
  if (!dispositionsByKind.get(kind).has(disposition)) throw new GuideError("INVALID_INPUT", `disposition ${disposition} is not valid for ${kind}`);
  ...
  return mutateEngagement(config, input.engagement_id, input.operation_id, input, "decision_recorded", (state) => {
    ...
    if (state.decisions.some(({ decision_id: id }) => id === decisionId)) throw new GuideError("DECISION_CONFLICT", `decision ${decisionId} already exists`);
    ...
    if (kind === "artifact_review") {
      ...
      if (!Number.isInteger(input.artifact_revision) || input.artifact_revision < 1) throw new GuideError("INVALID_INPUT", "artifact_review requires artifact_revision");
      const artifactDigest = requireString(input.artifact_digest, "artifact_digest", { max: 100 });
      if (!evidenceRefs.includes(artifactRef)) throw new GuideError("INVALID_INPUT", "artifact_ref must also appear in evidence_refs");
      const artifact = state.artifacts.find(({ artifact_id: id, revision }) => id === artifactRef && revision === input.artifact_revision);
      if (!artifact) throw new GuideError("REFERENCE_TYPE_MISMATCH", "artifact_ref must name a saved artifact");
      if (artifact.content_digest !== artifactDigest) throw new GuideError("ARTIFACT_DIGEST_MISMATCH", "artifact_digest does not match the requested immutable revision");
      ...
      if (input.decision_stream_id !== undefined) throw new GuideError("INVALID_INPUT", "artifact_review derives its decision stream from the exact artifact revision; omit decision_stream_id");
    } else if (input.artifact_ref !== undefined || input.artifact_revision !== undefined || input.artifact_digest !== undefined) {
      throw new GuideError("INVALID_INPUT", "artifact_ref, artifact_revision, and artifact_digest are supported only for artifact_review decisions");
    } else {
    ...
    if (heads.length === 0) {
      if (input.supersedes_decision_id !== undefined) throw new GuideError("DECISION_SUPERSESSION_CONFLICT", "the first decision in a stream cannot supersede another decision");
    } else {
      if (heads.length !== 1) throw new GuideError("DECISION_STREAM_AMBIGUOUS", `decision stream ${decisionStreamId} has multiple current heads`);
      ...
      if (supersedesDecisionId !== heads[0].decision_id) {
        throw new GuideError("DECISION_SUPERSESSION_CONFLICT", `supersedes_decision_id must name current decision ${heads[0].decision_id} in stream ${decisionStreamId}`);
      }
      if (heads[0].scope_digest !== scopeDigest) {
        throw new GuideError("DECISION_SCOPE_CONFLICT", "a decision stream cannot silently change scope; open a new stream for a different scope");
      }
    }
    const decision = {
