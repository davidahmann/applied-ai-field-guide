# Applied AI Delivery and Operating Model

This page connects field discovery, applied-AI engineering, product, platform, and operations into one delivery system. It applies to a team improving its own organization and to an FDE working with a customer. It separates portable lessons from Palantir, OpenAI, Anthropic, and the AI Engineer practitioner corpus; it does not claim that any organization follows one universal method.

Reconciling a brief with reality is substantive delivery work, not a preliminary architecture interview. Whoever leads discovery preserves the inherited brief, finds the person who actually knows the process, observes representative work, reconciles sold, stated, observed, system-enforced, and policy-authorized claims, and obtains a scoped human disposition when evidence changes the boundary. FDEs often carry this responsibility across a customer boundary; internal teams need it too. Use the [field-engagement playbook](../playbooks/00-field-engagement-and-reframing.md) and [engagement-reframe record](../templates/engagement-reframe.json) before silently converting a contradiction into requirements or architecture.
...
## Shared responsibilities, one delivery system

| Practice | Primary responsibility | Durable output |
| --- | --- | --- |
| Discovery and delivery leadership | Discover the real work, resolve scope conflicts, prove a narrow outcome, and establish operating capability | A business-owned workflow with measured value, adoption, and service ownership |
| Product and workflow ownership | Decide which problem is worth solving, what users may change, and how acceptance will be verified | A bounded outcome, usable work surface, adoption path, and owned investment decision |
...
## The applied-AI delivery method
...
## Separate workflow-specific context from reusable practice

Being inside one company does not erase access or purpose boundaries. A business unit's data, policy, and decisions remain governed even when the platform and delivery teams share an employer.
...
The boundary matters. Field learning should improve the product without extracting private data or turning one team's accidental workaround into a universal feature. A candidate pattern becomes reusable only after it is sanitized, recurrence is evidenced without unauthorized data transfer, an owner accepts it, and the normal design, evaluation, release, and rollback gates are satisfied. Use the [field-learning register](../templates/field-learning-register.md) to make that decision explicit. `FDE-004`.
...
| Workflow portfolio | Where should the organization invest, transfer, productize, build capacity, or exit? | [Workflow portfolio review](../templates/workflow-portfolio-review.md), decision rights, proof gates, cohort stage flow, time to accepted value, full delivery economics, target-specific effort, reuse, continuation, and capacity |
...
Applied-AI delivery teams can receive unusual access to operational context, organizational relationships, data, and decision paths. Preserve five boundaries:
...
- Service ownership, operation, and retirement are designed from the beginning; an internal delivery team may retain them only through explicit, resourced responsibility.
